...
def run_pipeline(topic: str, recipients: list[str] | None = None, run_id: str | None = None, dry_run: bool = True) -> Path:
    if not topic.strip():
...
def main() -> int:
    ...
    try:
        run_dir = run_pipeline(topic, recipients=recipients, run_id=args.run_id, dry_run=dry_run)
        print(run_dir)
        return 0
